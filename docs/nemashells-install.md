...
The METAXIS service runs in a hardened Docker container inside the OrbStack
Ubuntu machine and is published only on loopback. OrbStack forwards that
machine loopback to the Mac loopback; the app contains presentation state only.
...
Apple toolchain already present, installs it under `~/Applications`, copies a
release archive into the named OrbStack machine, installs Docker inside that
machine, builds the pinned container, and enables the systemd service.
...
- OrbStack Ubuntu: development runtime and Docker lifecycle.
